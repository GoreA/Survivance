...
//     To be more precise, these titles should be treated each.
    ...
    populateSurvivanceIndex(persons, 0.01f, 1500);

    populateSurvivance(personsMr, 0.005f, 1500);
    populateSurvivance(personsMrs, 0.001f, 1500);
    populateSurvivance(personsMiss, 0.0004f, 500);
    populateSurvivance(personsMaster, 0.005f, 500);
...
//Should obtain : 0.04705481; 0.3663616; 0.38562405
  ...
  private static void populateSurvivanceIndex(List<Person> persons, float alpha, int iterations) {
    Map<String, List<Person>> ticketsMap = persons.stream().collect(groupingBy(Person::getTicket));
    for (Map.Entry<String, List<Person>> e : ticketsMap.entrySet()) {
      long knownSurvivancePersons = e.getValue().stream().filter(p -> p.getSurvived() < 2).count();
      long sum = e.getValue().stream().filter(p -> p.getSurvived() == 1).count();
      if (e.getValue().size() > 1 && knownSurvivancePersons > 0) {
        double index = sum / knownSurvivancePersons;
        e.getValue().stream().forEach(p -> p.setSurvivanceIndex(index * 2 - 1));
      }
    }
  }
